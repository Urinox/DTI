// components/Division Head/Cards/TravelOrderCard.tsx
...
export default function TravelOrderCard({ info, onApprove, onDisapprove }: TravelOrderCardProps) {
    ...
    const displayStatus = info.status === 'Approved by Division Head' ? 'Approved' : info.status
    
    const isPending = info.status === 'Pending'
    const isApproved = info.status === 'Approved' || info.status === 'Approved by Division Head'
    ...
    const getStatusColor = (status: string) => {
        // ✅ Check for both 'Approved' and 'Approved by Division Head'
        if (status === 'Approved' || status === 'Approved by Division Head') {
        ...
        <div className='flex flex-col border-[2px] border-gray-600 rounded-lg items-center mx-10 py-4 gap-3'>
            <div className='flex justify-between items-center w-full px-5 pb-4 border-b-[1px] border-gray-300'>
                ...
                <div className='flex items-center gap-3'>
                    {/* Show Approve/Disapprove buttons only for Pending status */}
                    {isPending && (
                        <>
                            <input 
                    ...
                    {/* Show status badge for ALL statuses with mapped display */}
