// components/Division Head/Cards/OvertimeCard.tsx
        ...
        <div className='flex flex-col border-[2px] border-gray-600 rounded-lg items-center mx-10 py-4 gap-3'>
            <div className='flex justify-between items-center w-full px-5 pb-4 border-b-[1px] border-gray-300'>
                ...
                <div className='flex items-center gap-3'>
                    ...
                    {/* Show status badge for Pending Provincial (waiting for Provincial Director) */}
                    {isPendingProvincial && (
                        <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${getStatusColor(info.status)}`}>
                            Pending Provincial
                        </span>
                    )}
                    {/* Show status badge for Approved (final) */}
                    {isApproved && (
                        <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${getStatusColor(info.status)}`}>
                            Approved
                        </span>
                    )}
                    {/* Show status badge for Disapproved (final) */}
                    {isDisapproved && (
                        <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${getStatusColor(info.status)}`}>
                            Disapproved
                        </span>
