...
export default function TravelOrderContent({username, id, travelOrder, getTravelOrder}:{username: string, id: string, travelOrder: any[], getTravelOrder: () => void}) {
        ...
        <div className='flex flex-col w-full bg-gray-200'>
            ...
            <div className='flex flex-col my-5 mx-40 bg-white flex-1 rounded-xl border-[1] border-black'>
                ...
                <div className='flex flex-col py-5 gap-5'>
                    {travelOrder.map((info) => {
                        return(
                            filter == info.status || filter == 'All' ? <TravelOrderCard key={info.id} info={info}/> : ''
                        )
                    })}
                </div>
            </div>
            <div className={`justify-center items-center absolute w-full right-0 top-0 h-screen bg-gray-700/20 ${show ? 'flex' : 'hidden'}`}>
                <TravelOrderPopup showPopup={() => setShow(!show)} id={id} getTravelOrder={getTravelOrder}/>
            </div>
        </div>
    )
}